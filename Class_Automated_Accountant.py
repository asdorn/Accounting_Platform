class Accountant():
    ...
    def create_balance(self):
        ...
        while True:
            try:
                self.cash = float(input("Please enter the amount of Cash available: "))
            except ValueError:
        ...
        self.short_term_asset.append(["Cash", self.cash])
        self.short_assets_value_sum += self.cash
        ...
        self.short_assets_value_sum += self.inventory_value
        ...
        if value_of_inventory != 0:
            while True:
                try:
                    ...
                    break
            for searching_for_invent in self.short_term_asset:
        ...
        self.short_assets_value_sum += self.clients_to_pay
        ...
        self.long_assets_value_sum += self.permanent_possesion
        ...
        self.long_assets_value_sum += self.bank_trust
        ...
        self.short_liab_value_sum += self.short_term_loan

        while True:
            try:
                clients_to_be_payed = float(
                    input("Please enter the amount of money you are going to pay to your clients: "))
            except ValueError:
                print("Please enter a valid amount")
                continue
            else:
        ...
        self.short_liab_value_sum += self.client_to_be_payed
        ...
        self.short_liab_value_sum += self.expense_to_pay
        ...
        self.long_liab_value_sum += self.long_term_loans
        ...
        self.long_liab_value_sum += self.bonds

        # IF USER HAS OTHER LIABILITIES, APPENDING TO LISTS BASE ON KIND OF LIABILITY(LONG/SHORT)
        while True:
            try:
                more_liab = int(
                    input("Please enter the number of other kind of liabilities you have, if none, press 0: "))
            except ValueError:
                print("Please enter a valid number")
                continue
            if more_liab >= 2:
        ...
        self.equity_total_value_sum += self.shares_and_premia
        ...
        self.equity_total_value_sum += self.surpluses

        # IF USER HAS MORE KINDS OF EQUITY,APPENDING TO EQUITY LIST
        while True:
            try:
                more_equity = int(
                    input("Please enter the number of other kind of equities that you have, if none, press 0: "))
            except ValueError:
                print("Please enter a valid number")
                continue
            if more_equity >= 2:
        ...
        if more_equity > 0:
            ...
            for diverticulum_3 in range(more_equity):
                ...
                self.equity_total_value_sum += value_8
    ...
    def show_balance(self):
        ...
        print("\nAssets total sum:\t", self.short_assets_value_sum + self.long_assets_value_sum)
        ...
        print("\nLiabilities total sum:\t", self.short_liab_value_sum + self.long_liab_value_sum)

        print("\nEquity")
        for index_equity in self.equity:
            print(index_equity[0], "\t", index_equity[1])

        print("\nEquity total sum:\t", self.equity_total_value_sum, "\n")
        self.profit_loss()
        print("\nStatments in Equity")
        print("Issue:\t", self.sum_of_issue)
        print("Net Profit:\t", self.net_profit)
        print("Dividends distributed:\t", self.dividend)
        print("Equity end of year:\t", self.equity_total_value_sum)
    ...
    def finance_proportion(self):
        self.liab_total_value_sum = self.short_liab_value_sum + self.long_liab_value_sum
        self.assets_total_value_sum = self.short_assets_value_sum + self.long_assets_value_sum
        ...
        if self.equity_total_value_sum == 0:
            ...
            self.liab_to_equity_ratio = (self.liab_total_value_sum / self.equity_total_value_sum)
            print("liabilities to Equity ratio is", self.liab_to_equity_ratio, "\n")

    def dividends_distribution(self):
        dividends = float(input("How much Dividends Did you distributed to your Share Holders?"))
        self.dividend += dividends

        for looking_for_cash in self.short_term_asset:
            if looking_for_cash[0] == "Cash":
                self.cash -= dividends
                self.short_assets_value_sum -= dividends
                looking_for_cash[1] = looking_for_cash[1] - dividends

        for looking_for_surpluses in self.equity:
            if looking_for_surpluses[0] == "Surpluses":
                self.surpluses -= dividends
                self.equity_total_value_sum -= dividends
                looking_for_surpluses[1] = looking_for_surpluses[1] - dividends
    ...
    def selling_inventory(self):
        ...
        if will_clients_pay_now == "No":
            while True:
                try:
                    paying_later = float(
                        input("Please enter the amount of inventory units that you will be payed on later: "))
                except ValueError:
                    print("Please enter a valid amount")
                    continue
                else:
        ...
        for number_of_cycles in range(1, len(self.inventory_transaction) + 1):
            if len(self.inventory_transaction) > 0:
                if self.inventory_transaction[len(self.inventory_transaction) - number_of_cycles][0] == "Up":
                    last_inventory_buy_value = \
                    self.inventory_transaction[len(self.inventory_transaction) - number_of_cycles][1]
                    last_inventory_buy_amount = \
                    self.inventory_transaction[len(self.inventory_transaction) - number_of_cycles][2]
                    break
        ...
        for looking_for_cash in self.short_term_asset:
            if looking_for_cash[0] == "Cash":
                if paying_later > 0:
                    self.cash += sale_price - (paying_later * price_of_sell)
                    self.short_assets_value_sum += sale_price - (paying_later * price_of_sell)
                    looking_for_cash[1] = looking_for_cash[1] + sale_price - (paying_later * price_of_sell)
                else:
                    self.cash += sale_price
                    self.short_assets_value_sum += sale_price
        ...
        if paying_later > 0:
            for looking_for_clients in self.short_term_asset:
                if looking_for_clients[0] == "Clients to pay":
                    ...
                    self.short_assets_value_sum += (paying_later * price_of_sell)
        ...
        for looking_for_stock in self.short_term_asset:
            if looking_for_stock[0] == "Inventory":
                self.inventory_value -= value_of_inventory
                self.short_assets_value_sum -= value_of_inventory
                looking_for_stock[1] = looking_for_stock[1] - value_of_inventory

        for looking_for_surpluses in self.equity:
            if looking_for_surpluses[0] == "Surpluses":
                self.surpluses += sale_price
                self.equity_total_value_sum += sale_price
        ...
        for looking_for_surpluses_2 in self.equity:
            if looking_for_surpluses_2[0] == "Surpluses":
                ...
                self.equity_total_value_sum -= value_of_inventory
    ...
    def take_a_loan(self):
        ...
        if long_or_short == "short":
            for looking_for_short_loan in self.short_term_liab:
                if looking_for_short_loan[0] == "Short term loans":
                    ...
                    self.short_liab_value_sum += value_of_loan
        ...
        if long_or_short == "long":
            for looking_for_long_loan in self.long_term_liab:
                if looking_for_long_loan[0] == "Long term loans":
                    ...
                    self.long_liab_value_sum += value_of_loan
        ...
        for looking_for_cash in self.short_term_asset:
            if looking_for_cash[0] == "Cash":
                ...
                self.short_assets_value_sum += value_of_loan
    ...
    def issue(self):
        ...
        for looking_for_shares in self.equity:
            if looking_for_shares[0] == "Shares and Premia":
                ...
                self.sum_of_issue += value_of_issue
        ...
        self.equity_total_value_sum += value_of_issue
        self.short_assets_value_sum += value_of_issue
    ...
    def buying_permanent_possesion(self):
        ...
        for looking_for_cash in self.short_term_asset:
            if looking_for_cash[0] == "Cash":
                ...
                self.short_assets_value_sum -= possesion_value
        ...
        for looking_for_permanent_possesion in self.long_term_assets:
            if looking_for_permanent_possesion[0] == "Permanent Possesion":
                ...
                self.long_assets_value_sum += possesion_value
    ...
    def returning_expenses_to_pay(self):
        ...
        for looking_for_cash in self.short_term_asset:
            if looking_for_cash[0] == "Cash":
                ...
                self.short_assets_value_sum -= how_much_to_return
        ...
        for looking_for_expenses_to_be_payed in self.short_term_liab:
            if looking_for_expenses_to_be_payed[0] == "Expenses to be payed":
                ...
                self.short_liab_value_sum -= how_much_to_return
        ...
        while True:
            try:
                used_inventory = float(
                    input("Please enter the amount of Inventory that was used to create the new Inventory: "))
            except ValueError:
                print("Please enter a valid amount")
                continue
            else:
            ...
            for looking_for_cash in self.short_term_asset:
                if looking_for_cash[0] == "Cash":
                    if paying_now_or_future == "Yes":
                        ...
                        self.expense_to_pay += (new_inventory_amount * salary_per_unit) - (
                                    how_many_payed_now * salary_per_unit)
